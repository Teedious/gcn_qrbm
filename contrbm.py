...
class RBM:
    ...
    def learn(self, epochmax):
        ...
        for epoch in range(1,epochmax):
            ...
            # Debug messages
            if (epoch==1) or (epoch%10==0) or (epoch==epochmax):
    ...
    def wview(self):
        ...
        """
        import pylab as p
        p.plot(range(np.size(self.W[2])),self.W[2], 'bo')
        p.show()
...
if __name__ == "__main__":
    ...
    p.figure(1)
    p.plot(range(np.size(rbm.E)),rbm.E, 'b+')

    p.figure(2)
    p.plot(range(np.size(rbm.Err)),rbm.Err, 'r.')
    ...
    p.figure(7)
    p.plot(range(np.size(rbm.stat)), rbm.stat, "b.")

    p.figure(8)
    p.plot(range(np.size(rbm.stat2)), rbm.stat2, "b.")
