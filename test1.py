...
class Tester:
    def __init__(self, save_file=False, create_images=False, enc=None):
    ...
    def create_estimators(self, x_test, dataset):
        lr = 0.1
        iterations = 15
        classic_iterations = 50
        num_hidden = 10
        batch_size = 1000
        weight_decay = 0
        ...
        rbm1 = BernoulliRBM(tester=self, learning_rate=lr, n_iter=classic_iterations, n_components=num_hidden,
                            verbose=True, batch_size=batch_size, op_mode=BernoulliRBM.op_mode_classic)

        rbm2 = BernoulliRBM(tester=self, learning_rate=lr, n_iter=iterations, n_components=num_hidden,
                            verbose=True, batch_size=batch_size, op_mode=BernoulliRBM.op_mode_simulate_quantum)

        rbm3 = BernoulliRBM(tester=self, learning_rate=lr, n_iter=iterations, n_components=num_hidden,
                            verbose=True, batch_size=batch_size, op_mode=BernoulliRBM.op_mode_quantum)

        rbm4 = PCDRBM(tester=self, visible=self.dataset_categories[dataset], hidden=num_hidden, particles=-1,
                      iterations=classic_iterations, epochs=10000, step=lr, weight_decay=weight_decay)

        rbm5 = PCDRBM(tester=self, visible=self.dataset_categories[dataset], hidden=num_hidden, particles=-1,
                      iterations=classic_iterations, epochs=10000, step=lr, weight_decay=weight_decay)

        rbm6 = PCDRBM(tester=self, visible=self.dataset_categories[dataset], hidden=num_hidden, particles=-1,
                      iterations=iterations, epochs=10000, step=lr, weight_decay=weight_decay, op_mode=PCDRBM.op_mode_simulate_quantum)

        rbm7 = PCDRBM(tester=self, visible=self.dataset_categories[dataset], hidden=num_hidden, particles=-1,
                      iterations=iterations, epochs=10000, step=lr, weight_decay=weight_decay, op_mode=PCDRBM.op_mode_simulate_quantum)
    ...
    def save_weights_img(self, B, C, W, i):
        if self.save_file and self.create_images:
            ...
            Q = upper_diagonal_blockmatrix(
                B, C, W)
            heatmap = plt.imshow(Q, cmap='hot', interpolation='nearest')
    ...
    def compare_on_dataset(self, dataset, metrics_result_file, test_training_data):
        ...
        estimators = self.create_estimators(x_test, dataset)
    ...
    """.format(dataset))
                ...
                csv_file = "{}/{}_train_{}_metric_results.txt".format(
                    csv_dir, dataset, self.cur_test_train) if self.save_file else None
    ...
    def test_all(self):
        test(self.dataset_categories.keys(), [False, True])
